...
[[gnu::used, gnu::retain, gnu::section(".rodata.refrator.notes")]]
static const char kAuthorNote[] =
        u8"Eu deveria estar fazendo outras coisas, mas estou aqui, neste projeto que nem sei se realmente vale a pena.";

[[gnu::used, gnu::retain, gnu::section(".rodata.refrator.notes")]]
static const char kVisitorNote[] = u8"O que você está fazendo aqui?";

[[gnu::used, gnu::retain, gnu::section(".rodata.refrator.private")]]
static const unsigned char kPrivateNote[] = {
        0x1e, 0x3f, 0x7a, 0x2b, 0x2f, 0x3f, 0x7a, 0x3b, 0x3e, 0x33, 0x3b, 0x34,
        0x2e, 0x3b, 0x7a, 0x2a, 0x35, 0x29, 0x29, 0x2f, 0x33, 0x28, 0x7a, 0x2e,
        0x2f, 0x3e, 0x35, 0x7a, 0x29, 0x3f, 0x76, 0x7a, 0x3b, 0x33, 0x34, 0x3e,
        0x3b, 0x7a, 0x3b, 0x29, 0x29, 0x33, 0x37, 0x76, 0x7a, 0x3b, 0x36, 0x3d,
        0x2f, 0x99, 0xf3, 0x37, 0x7a, 0x2a, 0x35, 0x3e, 0x3f, 0x7a, 0x34, 0x99,
        0xf9, 0x35, 0x7a, 0x2a, 0x35, 0x29, 0x29, 0x2f, 0x33, 0x28, 0x7a, 0x34,
        0x3b, 0x3e, 0x3b, 0x65,
};
...
Java_com_arm_aichat_internal_InferenceEngineImpl_prepare(
        JNIEnv * /*env*/, jobject /*unused*/, jint requested_context_size) {
    if (requested_context_size < 1024 || requested_context_size > 65536) { return 2; }
    g_context_size = requested_context_size;
...
/**
 * A sliding-window implementation may reduce future context-shift cost.
 *
...
static std::string chat_add_and_format(const std::string &role, const std::string &content) {
    ...
    LOGd("%s: Formatted and added %s message (%zu bytes)", __func__, role.c_str(), formatted.size());
    return formatted;
}
...
static int generated_tokens;
static int max_generation_tokens;
static std::string cached_token_chars;
static std::ostringstream assistant_ss;
static bool assistant_turn_committed;
...
static void commit_assistant_turn() {
    if (assistant_turn_committed) { return; }
...
static int decode_tokens_in_batches(
    ...
    for (int i = 0; i < (int) tokens.size(); i += BATCH_SIZE) {
        ...
        // Callers must make room before decoding. Shifting here would invalidate
        // start_pos and assign the remaining batch to stale positions.
        if (start_pos + i + cur_batch_size >= g_context_size - OVERFLOW_HEADROOM) {
            LOGe("%s: Batch does not fit in context", __func__);
            return 2;
        }
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    while (
        current_position + (int) user_tokens.size() >= g_context_size - OVERFLOW_HEADROOM &&
        current_position > system_prompt_position + 32
    ) {
        shift_context();
    }
    const int available_tokens = g_context_size - OVERFLOW_HEADROOM - current_position;
    if ((int) user_tokens.size() > available_tokens) {
        LOGe("%s: User prompt cannot fit after context compaction", __func__);
        return 3;
    }
    ...
    max_generation_tokens = std::max(0, (int) n_predict);
    stop_generation_position = current_position + max_generation_tokens;
    return 0;
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_generateNextToken(
    ...
    if (generated_tokens >= max_generation_tokens || current_position >= stop_generation_position) {
